...
class XbeeControl:
    def __init__(self):
        ...
        self.XbeeCom = serial.Serial(
            self.PORT, self.BAUD_RATE
        )  # create the actual serial
    ...
    def SendCommand(self, newEvent: Event):
        ...
        match (newEvent.type):
            ...
            case pygame.JOYAXISMOTION:
                ...
                if (abs(newEvent.dict["value"]) < self.DEADBAND) and (
                    newEvent.dict["axis"] < 4
                ):
    ...
    def SendJoyPad(self, newEvent: Event):
        ...
        if x == 0 and y == 1:
            if (
                self.values[CommandCodes.BUTTONS.SELECT.index + 6] == 2
            ):  # left button is on
                self.reverseMode = False
                result = "reverse off"
            if (
                self.values[CommandCodes.BUTTONS.START.index + 6] == 2
            ):  # right button is on
                self.creepMode = False
